...
Pure functions that operate on a file descriptor.  No state, no classes.
Each function builds a 6-byte CDB (Command Descriptor Block), calls
sg_io() to execute it, and decodes the response into a Python dict or
typed value.
...
def inquiry(fd):
    ...
    """
    data = sg_io(fd, bytes([OP_INQUIRY, 0, 0, 0, 63, 0]), data_in_len=63)
...
def test_unit_ready(fd):
    ...
    """
    try:
        sg_io(fd, bytes([OP_TEST_UNIT_READY, 0, 0, 0, 0, 0]))
        return True
    except SCSIError:
        return False


def request_sense(fd):
    ...
    """
    data = sg_io(fd, bytes([OP_REQUEST_SENSE, 0, 0, 0, 10, 0]), data_in_len=10)
...
def mode_sense(fd):
    ...
    """
    data = sg_io(fd, bytes([OP_MODE_SENSE, 0, 0, 0, 61, 0]), data_in_len=61)
    return {
...
def mode_select(
    ...
    sg_io(fd, bytes([OP_MODE_SELECT, 0, 0, 0, 43, 0]), data_out=bytes(buf))


def set_color_tab(fd, channel, data):
...
def start_exposure(fd):
    ...
    """
    sg_io(fd, bytes([OP_DFRCMD, 0, SUB_START_EXPOSURE, 0, 0, 0]), timeout=60000)


def print_line(fd, line_no, color, pixels):
    ...
    sg_io(fd, cdb, data_out=payload)


def terminate_exposure(fd):
    ...
    to idle state.
    """
    sg_io(fd, bytes([OP_DFRCMD, 0, SUB_TERMINATE_EXPOSURE, 0, 0, 0]))
...
def stop_print(fd):
    ...
    """
    try:
        sg_io(fd, bytes([OP_STOP_PRINT, 0, 0, 0, 0, 0]))
    except SCSIError:
        pass


def current_status(fd):
    ...
    """
    data = sg_io(
        fd, bytes([OP_DFRCMD, 0, SUB_CURRENT_STATUS, 0, 7, 0]), data_in_len=7
    )
    return {
...
def film_name(fd, slot):
    ...
    try:
        data = sg_io(fd, cdb, data_in_len=24)
...
def upload_film_table(fd, slot, encrypted_data):
    ...
    sg_io(fd, cdb, data_out=payload, timeout=30000)
